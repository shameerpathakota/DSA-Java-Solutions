class Solution {
    ...
    public int[] maximumWeight(List<List<Integer>> intervals) {
        ...
        for(int i = 0; i < n; i++){
            Intervals[i][0] = intervals.get(i).get(0);
            Intervals[i][1] = intervals.get(i).get(1);
            Intervals[i][2] = intervals.get(i).get(2);
            Intervals[i][3] = i;
        }
    ...
    /*add the index values to the intervals.
    Now sort the new intervals based on the starting time.
    now preprocess the nextindex for each interval
    solve(intervals, 0, k) here k = 4 (given).
    */
